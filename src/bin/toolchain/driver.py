...
class Driver(WrapperXml):
    ...
    def fillAllTemplates(self):
        ...
        for component in project.getInstancesList():
            if component.getNum()=="0":
                driverT = component.getDriver_Template(os)
                if driverT != None:
                    display.msg("Copy and fill template for "+component.getName())
                    for templatefile in driverT.getTemplatesList():
                        try:
                            template = open(settings.projectpath+COMPONENTSPATH+"/"+\
                                            component.getInstanceName()+"/"+\
                                            DRIVERS_TEMPLATES_PATH+"/"+os+"/"+\
                                            templatefile,"r")
                            destfile = open(settings.projectpath+DRIVERSPATH+"/"+\
                                            component.getName()+"/"+templatefile,"w")
                        except IOError,e:
                            raise Error(str(e),0)
    ...
    def fillTemplateForEachInstance(self,template,component):
        ...
        project = self.project
        out = ""
        for instance in project.getInstanceListofComponent(component.getName()):
            for writeline in template.split("\n"):
                ...
                for generic in iterator:
                    writeline = re.sub(r'\/\*\$generic:'+generic.group(1)+r'\$\*\/',
                ...
                for interface in iterator:
                    writeline = re.sub(r'\/\*\$registers_base_address:'+interface.group(1)+'\$\*\/',
                ...
                for match in iterator:
                    ...
                    if not attributevalue :
                        raise Error("Wrong register value -> "+\
                                ...
                                ":"+match.group(3)+"\n",0)
                    writeline = re.sub(r'\/\*\$register:'+match.group(1)+\
                                        ':'+match.group(2)+':'+match.group(3)+'\$\*\/',
                            attributevalue,writeline)
                # interrupt_number
                if re.search(r'\/\*\$interrupt_number\$\*\/',writeline)!=None:
                    interruptlist = instance.getInterruptList()
                    if len(interruptlist) == 0:
                        raise Error("No interruption port in "+instance.getInstanceName(),0)
                    ...
                    if len(connect) == 0:
                        raise Error("Interrupt "+interruptport.getName()+" is not connected",
                                        0)
                    elif len(connect) > 1:
                        display.msg("More than one connection for interruption port "\
    ...
    def fillTemplate(self,template,destfile,component):
        ...
        for line in template:
            if state == "STANDARD" :
                begintag = re.match('^\/\*\$foreach\:instance\$\*\/',line) #match begining tag
                if begintag != None:
                    state = "FOREACH_INSTANCE"
                    foreach_template = ""
                    continue
                # number_of_instances
                if re.search(r'\/\*\$number_of_instances\$\*\/',line) != None:
                    listOfInstances = project.getInstanceListofComponent(component.getName())
                    line = re.sub(r'\/\*\$number_of_instances\$\*\/',
                                        str(len(listOfInstances)),
                                        line)
                # main clock speed
                if re.search(r'\/\*\$main_clock\$\*\/',line) != None:
                    frequency = project.getPlatform().getMainClock()
                    line = re.sub('\/\*\$main_clock\$\*\/',frequency,line);
                destfile.write(line)
            elif state == "FOREACH_INSTANCE":
                endtag = re.match('^\/\*\$foreach\:instance\:end\$\*\/',line) # match end tag
                if endtag != None:
                    state = "STANDARD"
                    destfile.write(self.fillTemplateForEachInstance(foreach_template,component))
                else:
    ...
    def setBSPOperatingSystem(self,operatingsystem):
        ...
        if self.getNode(nodename="bsp") != None:
            self.getNode(nodename="bsp").setAttribute("os",operatingsystem)
        else:
