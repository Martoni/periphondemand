...
def renameFile(oldfilepath,newfilepath):
    ...
    try:
        ...
        raise Error(str(e)+"\nrenaming "+str(oldfilepath)+" in "+str(newfilepath))
...
def copyDirectory(source,target):
    ...
    # code from http://tarekziade.wordpress.com/2008/07/08/shutilcopytree-small-improvement/
...
def listFileType(dirpath,ext):
    ...
    return [filename for filename in listfile if filename.split(".")[-1]==ext]
...
def deleteAllDir(dirpath):
    dirpath = os.path.expanduser(dirpath)
    try:
        for dir in listDirectory(dirpath):
            delDirectory(dirpath+"/"+dir)
    except IOError,e:
        raise Error(str(e),0)
