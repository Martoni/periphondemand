/*
 * Linux kernel platform informations for the POD component pod_pwm
 *
...
#include "../../virtual_components/pod_pwm/pod_pwm.h"
...
static struct resource /*$instance_name$*/_resources[] = {
	[0] = {
		...
		.end = ARMADEUS_FPGA_BASE_ADDR + /*$instance_name$*/_BASE + 0x04,
...
static int __init pod_pwm_platform_init(void)
{
	int ret;
...
/*$foreach:instance$*/
	...
	if (ret < 0)
		printk(KERN_ERR "Error: Can't init device pod_pwm\n");
/*$foreach:instance:end$*/

		return ret;
}
module_init(pod_pwm_platform_init);

static void __exit pod_pwm_platform_exit(void)
{
/*$foreach:instance$*/
	platform_device_unregister(&/*$instance_name$*/_pdev);
/*$foreach:instance:end$*/
}
module_exit(pod_pwm_platform_exit);

MODULE_AUTHOR("Kevin Joly <joly.kevin25@gmail.com>");
MODULE_DESCRIPTION("Platform driver for pod_pwm driver");
