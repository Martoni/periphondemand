...
//		printk(KERN_DEBUG "STATE_WRITE pos=%d len=%d\n",i2c->pos,msg->len);
...
static int ocores_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	...
	//printk(KERN_DEBUG "nmsgs %d, msg->len %d\n",num,i2c->msg->len);
...
static int __devinit ocores_i2c_probe(struct platform_device *pdev)
{
	 ...
	 * check if ID correct
	 *****************************/
	data = ioread16(i2c->base +pdata->idoffset);
	if(data != pdata->id){
		printk(KERN_ERR "ID is incorrect, is device present ?\n");
		goto id_failed;
	}
