...
def addressdecoding(masterinterface,sysconinterface,intercon):
    ...
    for slave in masterinterface.getSlavesList():
        ...
        if slavesizeaddr == 1:
            out=out+TAB+slavename_addr+" <= "+masteraddressname+"(1);\n"
    ...
    out = out+TAB+"decodeproc : process("+clk_name+","+rst_name+\
            ","+masteraddressname+")\n"
    out = out+TAB+"begin\n"
...
def generateIntercon(masterinterface,intercon):
    ...
    ###########################
    #address decoding
    VHDLcode = VHDLcode + addressdecoding(masterinterface,sysconinterface,intercon)
    ###########################
    #controls slaves
