...
class Quartus(SynthesisWrapper):
    ...
    def addpinconstraints(self, connect, port):
        ...
        if port.position is not None:
            ...
            out += ';\n'
        out += "set_instance_assignment "
        out += '-name IO_STANDARD "'
        if portdest.standard is not None:
            out += portdest.standard
        else:
            out += port.standard
        out += '" -to ' + dest_conn
        if tag is not None:
    ...
    def generate_sdc(self, filename=None):
        ...
        out += "#*********************\n"
        out += "# Create Generated Clock\n"
        out += "#*********************\n"
        out += "derive_pll_clocks\n"
        out += "#*********************\n"
        out += "# Set Clock Uncertainty\n"
        out += "#*********************\n"
        out += "derive_clock_uncertainty\n"
        try:
